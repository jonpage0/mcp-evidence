...
/* Pull event-level metadata from TT_Events and TT_Events_Series */
event_details AS (
  SELECT
    e.id AS event_id,
    e.event_series_id,
  ...
  FROM maniac.tt_events e
  LEFT JOIN maniac.tt_events_series es 
         ON e.event_series_id = es.id
  WHERE
    (e.start->>'unix')::numeric >= 1727740800  -- Oct 1, 2024
...
ticket_counts AS (
  ...
  FROM maniac.tt_issued_tickets it
